...
/**
 * Clase test para la clase AddAdInteractorImpl.
 *
 * @author Robert Ene
 * @see com.robert.android.lostpets.domain.interactors.impl.AddAdInteractorImpl
 */
@RunWith(MockitoJUnitRunner::class)
class AddAdInteractorTest {
    ...
    @Mock
    private lateinit var mCallback: AddAdInteractor.Callback
    @Mock
    ...
    fun setUp() {
        ...
        mAd = Ad(100, "CODE1", Date(), AdStatus.ENABLED, PetStatus.LOST,
    ...
    @Test
    @Throws(Exception::class)
    fun testAddedAdSuccess() {
        val response = MockResponse()
        response.setBody(mGson.toJson(true))
        mMockWebServer.enqueue(response)

        AddAdInteractorImpl(mExecutor, mMainThread, mCallback, mContext, mService, mAd, mFile).run()

        verify(mCallback, timeout(RESPONSE_TIMEOUT)).onAddedAd(true)
        verifyNoMoreInteractions(mCallback)
    }

    @Test
    @Throws(Exception::class)
    fun testAddedAdFailure() {
        val response = MockResponse()
        ...
        verify(mCallback, timeout(RESPONSE_TIMEOUT)).onAddedAd(false)
        verifyNoMoreInteractions(mCallback)
    }
